...
entity Write_addresses_WithError is
  generic (
...
architecture rtl of Write_addresses_WithError is
  ...
  begin
    ...
    if (reset_n = '0') then
      ...
      case state is
        ...
        when NEXT_ADDR =>
          if(addr_cnt = "1000010010000000") then    -- Random address to give wrong value (the one after this is the one changed)
            addr_cnt   <= addr_cnt + 1;
            write_data <= "1000010010000001";       -- Data written to address is the same number as the address
            write_data_buf <= write_data;
            state <= WRITE_START;
          elsif(addr_cnt = "1000010010000001") then -- Return to regular pattern
            addr_cnt   <= addr_cnt + 1;
